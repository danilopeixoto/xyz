...
// Load 32-bit linear RBG texture to OpenGL
GLuint loadTexture(size_t width, size_t height, const std::vector<glm::vec3> & pixels) {
    GLuint textureID;
...
int main(int argc, char ** argv) {
    ...
    if (!readTexture("../res/textures/checkboard.ppm", width, height, pixels)) {
        glfwTerminate();
    ...
    GLuint textureID = loadTexture(width, height, pixels);
